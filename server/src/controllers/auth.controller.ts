...
export const refresh = async (req: Request, res: Response) => {
  ...
  // Atomically check the token is active/unexpired in DB and revoke it, so a
  // rotated or already-used token can never be replayed (and concurrent
  // refreshes with the same token can't both succeed).
  const hashRT = hashTokenCrypto(refreshToken);
  const storedToken = await findAndRevokeRefreshToken(hashRT);
