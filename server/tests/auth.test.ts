...
const mockFindAndRevokeRefreshToken = jest.fn<any>();
...
jest.unstable_mockModule('#src/services/token.service.ts', () => ({
  ...
  findAndRevokeRefreshToken: mockFindAndRevokeRefreshToken,
...
describe('Auth routes', () => {
  ...
  describe('GET /api/auth/refresh', () => {
    ...
    it('returns 401 when refresh token is not stored', async () => {
      ...
      mockHashTokenCrypto.mockReturnValue('hashed-refresh');
      mockFindAndRevokeRefreshToken.mockResolvedValue(null);
    ...
    it('rotates tokens and returns 200 on success', async () => {
      mockVerifyRefreshToken.mockResolvedValue('user-1');
      mockHashTokenCrypto.mockReturnValue('hashed-refresh');
      mockFindAndRevokeRefreshToken.mockResolvedValue({ sessionId: 'old-session' });
      ...
      expect(mockFindAndRevokeRefreshToken).toHaveBeenCalledWith('hashed-refresh');
      expect(mockSaveToCookie).toHaveBeenCalledWith(
        expect.anything(),
        'refresh-2',
        'access-2'
    ...
    it('does not touch other refresh-token rows when rotating', async () => {
      mockVerifyRefreshToken.mockResolvedValue('user-1');
      mockHashTokenCrypto.mockReturnValue('hashed-refresh');
      mockFindAndRevokeRefreshToken.mockResolvedValue({ sessionId: 'old-session' });
      mockCreateRandomToken.mockReturnValue('new-session');
